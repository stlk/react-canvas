...
var Surface = React.createClass({
  ...
  render: function () {
    ...
    return (
      React.createElement('canvas', {
        ...
        onMouseOut: debounce(this.handleMouseOut, 30, {maxWait: 100})})
    );
  },
  ...
  handleMouseMove: function (e) {
    if(this.props.onMouseMove && (Math.abs(this._lastMousePosition.x - e.pageX) > this.props.moveThreshold || Math.abs(this._lastMousePosition.y - e.pageY) > this.props.moveThreshold)) {
      this._lastMousePosition = {x: e.pageX, y: e.pageY};

      var rootNode = this.getDOMNode();
      var rootNodeBox = rootNode.getBoundingClientRect();
      var touchX = e.pageX - rootNodeBox.left;
      var touchY = e.pageY - rootNodeBox.top;
      this.props.onMouseMove({x: touchX, y: touchY});
    }
  },

  handleMouseOut: function (e) {
    if(this.props.onMouseMove) {
      this.props.onMouseMove({x: -1, y: -1});
    }
  },
